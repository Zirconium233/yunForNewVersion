...
~~**有人因为结束链触发未实现的人脸补验分支，强发finish包被判软件作弊了(猜测)。当前的实现带有大量fallback，通过未知情况立即终止不发finish包规避风险。如果测试过程遇到问题，请在issues里面反馈日志，千万不要修改代码强制发包！！！**~~

已由Astra确认finish包不发送是脚本误判，客户端`CheckRunStateDialog`将`url`作为状态图片、`list`作为条件明细展示；因此字段非空不代表需要补拍或复核，实际finish包参数正确，并且已有“跑步合格”先例。详见[issue78](https://github.com/Zirconium233/yunForNewVersion/issues/78) 软件作弊判定原因尚不能确定是之前的多次失败、多端登录和AI误操作等的累计还是当前实现问题。因此等有更多“跑步成功”案例后合并进入master分支。
